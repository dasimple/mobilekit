...
namespace DaSimple.MobileKit.Storing.Collections
{
	public abstract class VirtualCollection
	{
		...
		public virtual int Add(StoreDictionary[] records)
		{
		...
		{
			StoreDictionary[] records = new StoreDictionary[1] { record };
			return Add(records);
		}
		...
		public virtual StoreDictionary[] GetMultiple(StoreList columns, StoreDictionary where = null, int limit = 1)
		{
			return Store.Get(table, columns, where, limit);
		}

		public virtual StoreDictionary[] GetMultiple(StoreList columns, string whereColumn, string whereValue, int limit = 1)
		{
		...
		public virtual string[] GetMultiple(string column, StoreDictionary where = null, int limit = 1)
		{
			return Store.Get(table, column, where, limit);
		}

		public virtual string[] GetMultiple(string column, string whereColumn, string whereValue, int limit = 1)
		{
			StoreDictionary where = new StoreDictionary(whereColumn, whereValue);
		...
		{
			StoreDictionary[] records = GetMultiple(columns, where);
		...
		{
			string[] records = GetMultiple(column, where);
			return records.Length > 0 ? records[0] : null;
		}
